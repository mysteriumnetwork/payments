...
contract Topup is ERC820aClient {
    ...
    constructor(address _identity, address _token) public {
...
contract IdentityRegistry is Ownable {
    ...
    mapping(address => Identity) registeredIdentities;
    ...
    function registerIdentity(bytes32 _pubKeyPart1, bytes32 _pubKeyPart2, bytes memory _signature) public returns (bool) {
        ...
        registeredIdentities[_identity].topupAddress = address(new Topup(_identity, address(token)));
    ...
    function claimPromise(address _issuer, address _receiver, bytes32 _extraDataHash, uint256 _amount, bytes memory _signature) public returns (bool) {
        // Check if signature is valid, remember last running sum
        address _identity = keccak256(abi.encodePacked(ISSUER_PREFIX, _issuer, _receiver, _extraDataHash, _amount)).recover(_signature);
    ...
    function topUp(address _identity, uint256 _amount) public returns (bool) {
        ...
        token.operatorSend(msg.sender, address(this), _amount, "", "");

        emit ToppedUp(_identity, _amount, balances[_identity]);
        return true;
    }

    // TODO add timelock to delay withdrawals for 24h
    function withdraw(address _beneficiary, uint256 _amount, uint256 _nonce, bytes memory _signature) public returns (bool) {
        // Extract identity which signed this request
        address _identity = keccak256(abi.encodePacked(WITHDRAW_PREFIX, _beneficiary, _amount)).recover(_signature);
        require(_identity != address(0));
        ...
        require(_nonce > registeredIdentities[_identity].widthdrawalNonce);
        registeredIdentities[_identity].widthdrawalNonce = _nonce;

        emit Withdrawn(_identity, _amount, balances[_identity]);
        return true;
    }

    function recoverIdentity(bytes32 _pubKeyPart1, bytes32 _pubKeyPart2, bytes memory _signature) private pure returns (address) {
    ...
    function getPublicKey(address _identity) public view returns (bytes32, bytes32) {
        PublicKey storage _pubKey = registeredIdentities[_identity].publicKey;
    ...
    function transferCollectedFeeTo(address _beneficiary) public onlyOwner returns (bool) {
        ...
        collectedFee = 0;
        token.send(_beneficiary, _transferAmount, "");
        return true;
    }
}
