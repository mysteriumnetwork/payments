...
func newFailingHashAt(n int) hash.Hash {
...
func TestNewTree(t *testing.T) {
	tree := NewTree(NewSimpleHash, nil)
...
func TestNewTreeWithOpts(t *testing.T) {
	tree := NewTree(NewSimpleHash, &TreeOptions{EnableHashSorting: true, DisableHashLeaves: true})
	verifyInitialState(t, tree)
...
func TestTree_GenerateSingleLeaf(t *testing.T) {
	...
	treeHashedLeaves := NewTree(sha256.New, nil)
...
func TestHashOrderedTreeGenerate(t *testing.T) {
	...
	err := tree.Generate(data)
	assert.Nil(t, err)
	verifyGeneratedTree(t, tree, NewSimpleHash())
...
func TestTreeGenerateProof(t *testing.T) {
	tree := NewTree(sha256.New, &TreeOptions{
	...
	err := tree.Generate(data)
	assert.Nil(t, err)
	verifyGeneratedTree(t, tree, sha256.New())

	proofs, err := tree.GetProof(data[0])
	assert.NoError(t, err)
	assert.Len(t, proofs, 4)

	gotleft := hex.EncodeToString(tree.Root().Left.Hash) == hex.EncodeToString(proofs[len(proofs)-1])
	gotright := hex.EncodeToString(tree.Root().Right.Hash) == hex.EncodeToString(proofs[len(proofs)-1])
	assert.True(t, gotleft || gotright)
}

func TestGenerateFailedHash(t *testing.T) {
	tree := NewTree(newFailingHash, nil)
...
func generateBenchmark(b *testing.B, data [][]byte, fn HashFunc) {
	tree := NewTree(fn, nil)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		tree.Generate(data)
